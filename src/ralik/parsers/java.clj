...
(defgrammar java
  ...
  ;;
  (type-declaration
   (g| (class-or-interface-declaration)
       \;))
  ;;
  ...
  ;;
  (annotation
   (g \@ (qualified-identifier) (g? \( (g? (annotation-element)) \))))
  ;;
  (annotation-element
  ...
  (for-control
   (g| (for-var-control)
       (g (for-init) \; (g? (expression)) \; (g? (for-update)))))
  ;;
  (for-var-control
  ...
  (enum-body
   (g \{
      (g? (enum-constants)) (g? \,) (g? (enum-body-declarations))
      \}))
  ;;
  (enum-constants
   (g_ (enum-constant) \,))
